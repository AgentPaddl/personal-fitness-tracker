...
- `ios/EntraAuthKit/` is a second local Swift package (also a package dependency of the `Trainingsplan` target) holding the MSAL-independent Microsoft Entra ID token-acquisition orchestration (silent-first, interactive-fallback over the small `EntraTokenAcquiring` protocol), independently testable via `swift test` with no MSAL dependency, no network, and no Microsoft/Azure endpoint contacted. It depends on `FoodAnalysisKit` (for `AccessTokenProviding`) but never the reverse. The only MSAL-dependent code is `ios/Trainingsplan/Entra/MSALEntraTokenAcquirer.swift` in the app target.
...
- **Authentication token**: the iOS app does not send a static backend secret. It uses a short-lived Microsoft Entra ID access token via `AccessTokenProviding` and sends it as `Authorization: Bearer <token>` in `FoodAnalysisService`. The real implementation is `EntraAuthServiceFactory.configuredProviderOrNil()` (app target, `ios/Trainingsplan/EntraAuthService.swift`), which combines the MSAL-independent, unit-tested orchestration in the local `EntraAuthKit` package (`ios/EntraAuthKit/`) with the concrete MSAL adapter (`ios/Trainingsplan/Entra/MSALEntraTokenAcquirer.swift`, the only file that imports MSAL). It reads non-secret public config values (tenant ID, client ID, API scope, redirect URI) from Info.plist via `EntraConfiguration.load(bundle:)`. In `DEBUG` builds, missing configuration returns `nil` (no provider, no `Authorization` header - existing local-development behavior, unchanged). In `Release` builds, missing/invalid configuration fails closed (`FailClosedAccessTokenProvider`, every request throws `authenticationRequired` rather than silently sending unauthenticated requests). A production build must not contain a client secret or a static backend API key; it relies on Entra ID + Azure App Service Authentication (Easy Auth) instead. See `docs/architecture.md`'s Entra/Azure setup checklist for the exact external configuration still required (app registrations, redirect URI, Easy Auth) before this can be used against a real tenant.
...
- Run `swift test` in `ios/FoodAnalysisKit/` for the networking/validation/persistence-orchestration logic, and in `ios/EntraAuthKit/` for the Entra ID token-acquisition orchestration logic; run relevant app-level checks manually when automated coverage is absent (e.g. no XCTest target exists for the `Trainingsplan` app target itself - keep new business/networking/auth-orchestration logic in a local package instead, where it's testable).
