...
- **API key** (`FoodAnalysisKit.APIConfiguration.resolveBackendAPIKey()`, `API_KEY` environment variable, mirrors `API_BASE_URL`'s resolution mechanism): sent as `X-API-Key` on every request once set. `nil` is valid - the backend's own development-mode bypass doesn't require it; a production backend fails closed server-side (401) if the header is absent, so the client has no separate fail-closed requirement of its own. **Known gap**: there is currently no configuration mechanism for a genuinely installed production build (no Xcode scheme attached) to supply this secret - environment variables only work for Xcode-launched (Debug/physical-device-development) runs. A real installed-production deployment needs either a minimal Keychain-backed entry screen (not implemented - out of scope for this phase, and secrets must never go into Info.plist/xcconfig) or another mechanism decided before a real public/production rollout.
