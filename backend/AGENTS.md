...
- **Authentication (Phase 6):** the route requires either `APP_ENV=development` (local-dev bypass, unchanged) or a valid Microsoft Entra ID sign-in that Azure App Service/Functions has already authenticated through Easy Auth (`security.py::caller_is_authenticated`). In production the backend trusts only Azure's injected `X-MS-CLIENT-PRINCIPAL-ID` when `EASY_AUTH_ENABLED=true`; otherwise it fails closed. There is no iOS static backend secret and no `X-API-Key` header in the accepted production design.
...
- `function_app.py` calls `config.validate_config()` at import time so invalid configuration - a malformed/out-of-bounds gateway URL/timeout, a missing `GATEWAY_SERVICE_TOKEN`, a `localhost` gateway URL, or a non-HTTPS gateway URL - fails at startup rather than at first request. The backend also enforces the server-side Easy Auth enablement flag (`EASY_AUTH_ENABLED`) before trusting any Azure identity header.
- `GET /api/health` = process alive, nothing more (never reveals the gateway URL or other configuration). `GET /api/readiness` = the gateway and its dependencies are ready right now (calls the gateway's own anonymous `/readyz`, never a billed AI call, 3s timeout); returns 503 `not_ready` if unreachable or dependencies unavailable.
